...
import { ExpenseCategory, EXPENSE_CATEGORIES, formatCurrency, ExpenseEntry } from "@/lib/finance";
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-8 animate-in fade-in duration-500 ease-out">
      ...
      {showAddForm && (
        <section className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm animate-in slide-in-from-top-4 duration-300">
          <h2 className="text-base font-semibold text-slate-800 mb-4">Add New Expense</h2>
      ...
      <section className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          {expenses.length === 0 ? (
            ...
            <table className="min-w-full text-left text-sm whitespace-nowrap">
              ...
              <tbody className="divide-y divide-slate-100">
                {[...expenses].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map((expense: ExpenseEntry) => {
                  const catConfig = EXPENSE_CATEGORIES.find(c => c.name === expense.category);
                  return (
                    <tr key={expense.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-4 text-slate-500 font-medium">{expense.date}</td>
                      <td className="px-6 py-4">
                        <span className={clsx("inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium", catConfig?.bg, catConfig?.color)}>
                          {expense.category}
                        </span>
                      </td>
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
          <span className="text-xs italic text-slate-500">Showing {expenses.length} transactions</span>
